...
class Grid_File_Reader;


template <size_t dim>
class Grid_File_Reader<Gmsh, dim>
{
	using Space_Vector = EuclideanVector<dim>;

public:
	Grid_File_Reader(void) = delete;

	static Grid_Raw_Data<dim> read(const std::string& grid_file_name);
...
template <size_t dim>
Grid_Raw_Data<dim> Grid_File_Reader<Gmsh, dim>::read(const std::string& grid_file_name) {
...
template <size_t dim>
std::vector<EuclideanVector<dim>> Grid_File_Reader<Gmsh, dim>::make_node_grid_data(const Text& node_text) {
...
template <size_t dim>
std::array<std::vector<ElementGridData>, 3> Grid_File_Reader<Gmsh, dim>::make_element_data(const Text& element_text, const Text& physical_name_text) {
...
template <size_t dim>
Text Grid_File_Reader<Gmsh, dim>::read_about(std::ifstream& grid_file_stream, const std::string& target) {
