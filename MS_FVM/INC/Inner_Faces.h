...
template <size_t space_dimension>
class Inner_Faces_FVM_Linear_Base : public Inner_Faces_FVM_Base<space_dimension>
{
private:
    using Space_Vector = EuclideanVector<space_dimension>;
...
public:
    Inner_Faces_FVM_Linear_Base(Grid<space_dimension>&& grid);
...
template<typename Governing_Equation, typename Gradient_Method>
class Inner_Faces<Governing_Equation, FVM, MLP_u1<Gradient_Method>> : public Inner_Faces_FVM_Linear_Base<Governing_Equation::space_dimension()>
{
private:
    static constexpr size_t space_dimension_ = Governing_Equation::space_dimension();
...
template <size_t space_dimension>
template<typename Numerical_Flux_Function, typename Residual, typename Solution>
void Inner_Faces_FVM_Base<space_dimension>::calculate_RHS(std::vector<Residual>& RHS, const std::vector<Solution>& solutions) const {
...
template <size_t space_dimension>
Inner_Faces_FVM_Linear_Base<space_dimension>::Inner_Faces_FVM_Linear_Base(Grid<space_dimension>&& grid) : Inner_Faces_FVM_Base<space_dimension>(std::move(grid)) {
    SET_TIME_POINT;
    ...
    Log::content_ << std::left << std::setw(50) << "@ Construct Inner faces FVM Linear Base" << " ----------- " << GET_TIME_DURATION << "s\n\n";
    Log::print();
};
...
template <size_t space_dimension>
template<typename Numerical_Flux_Function, typename Residual, typename Solution, typename Gradient>
void Inner_Faces_FVM_Linear_Base<space_dimension>::calculate_RHS_with_gradient(std::vector<Residual>& RHS, const std::vector<Solution>& solutions, const std::vector<Gradient>& solution_gradients) const {
    for (size_t i = 0; i < this->num_inner_face_; ++i) {
        const auto [oc_index, nc_index] = this->oc_nc_index_pairs_[i];
