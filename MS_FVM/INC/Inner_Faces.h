...
class Inner_Faces<FVM, Constant_Reconstruction, Numerical_Flux_Function>
{
    static_require(ms::is_numeirical_flux_function<Numerical_Flux_Function>, "Wrong numerical flux");

    using Space_Vector_  = typename Numerical_Flux_Function::Governing_Equation::Space_Vector;
...
private:
    ...
    const std::vector<Space_Vector_> normals_;
    const std::vector<std::pair<size_t, size_t>> owner_neighbor_cell_container_indexes_;
    const std::vector<double> areas_;

public:
    Inner_Faces(Grid_Info_Extractor<FVM, Constant_Reconstruction, dimensions_>::Inner_Face_Infos&& inner_face_grid_info);
...
//    //void calculate_RHS(std::vector<Residual>& RHS, const std::vector<Solution>& solutions, const std::vector<Gradient_>& gradients) const;
...
template <typename Numerical_Flux_Function>
Inner_Faces<FVM, Constant_Reconstruction, Numerical_Flux_Function>::Inner_Faces(Grid_Info_Extractor<FVM, Constant_Reconstruction, dimensions_>::Inner_Face_Infos&& inner_face_info)
    : normals_(std::move(inner_face_info.normals)), owner_neighbor_cell_container_indexes_(std::move(inner_face_info.owner_neighbor_container_indexes)), areas_(std::move(inner_face_info.areas)) {
    this->num_inner_face_ = this->normals_.size();
};
