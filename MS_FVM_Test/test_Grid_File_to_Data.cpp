...
GTEST_TEST(Grid_File_To_Data_Gmsh, read_about){
	...
	const auto node_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "Nodes");
	//std::cout << node_text;

	const auto element_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "Elements");
	//std::cout << element_text;

	const auto physical_name_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "PhysicalNames");
...
GTEST_TEST(Grid_File_To_Data_Gmsh, make_node_data) {
	...
	const auto node_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "Nodes");
	const auto node_data_set = Grid_File_To_Data<Gmsh, 2>::make_node_grid_data(node_text);
...
GTEST_TEST(Grid_File_To_Data_Gmsh, cell_data) {
	...
	const auto element_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "Elements");
	const auto physical_name_text = Grid_File_To_Data<Gmsh, 2>::read_about(grid_file_stream, "PhysicalNames");
	const auto element_data_set = Grid_File_To_Data<Gmsh, 2>::make_element_data(element_text, physical_name_text);
